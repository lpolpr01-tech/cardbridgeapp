...
- `components/CreditProfile.tsx` — Full credit intelligence section: `CreditProfileSection` (parent, accepts `onSimulate` → opens AI Agent), `CreditScoreOverviewCard` (color gradient bar, per-bureau scores with change chips, "Simulate Score Changes" button), `CreditHealthCard`, `DebtSummaryCard`, `ScoreFactorsCard`, `PersonalizedRecommendationsCard`. Drill-down modals: `UtilizationModal` (per-card utilization), `PaymentHistoryModal` (calendar), `DebtDrillDownModal` (breakdown pie bar). Bureau layout: Equifax+Experian top row, TransUnion centered below. Supports loading/success/partial/no_data/error states, skeleton shimmer loaders, FadeSlideIn entrance animations.
...
- `app/(tabs)/index.tsx` — Card List screen; renders BalanceHeader → WalletCardStack → SubscriptionsRow → CreditProfileSection → SupportSection. FAB (FloatingBubbleMenu): 3 bubbles — Budgeting (bar-chart-2, green), AI Agent (cpu, purple), Notifications (bell, blue). Unread badge on notification bubble + FAB. AiAgentModal: real Claude claude-sonnet-4-6 via /api/ai-agent, context injection, 6 starter prompts, typing indicator, clear chat, disclaimer. NotificationsModal: 8 rich notifications, mark-read per item, mark-all-read, clear-all, empty state. BudgetingScreen: 6-tab budgeting modal.
- `components/BudgetingScreen.tsx` — Full budgeting modal: 6 tabs: Overview (donut chart, month selector, stats), Categories (50/30/20), Goals (custom goals), Debt (Avalanche/Snowball planner), Insights (tips), Settings (income/framework/alerts)
...
- `src/routes/auth.ts` — Beta login: POST /api/auth/login (hardcoded luispol/12345 → session token); plus Replit Auth OIDC routes kept for future use: GET /api/login, GET /api/callback, GET /api/logout, GET /api/auth/user
...
- Auth: Beta mode — hardcoded credentials (luispol/12345). `POST /api/auth/login` creates a PostgreSQL-backed session and returns a token. Frontend stores the token in `sessionStorage` (clears on tab close). `BetaLoginGate` in `_layout.tsx` blocks all screens until authenticated. TODO: Replace with Replit Auth before going live.
- Auth files: `context/AuthContext.tsx` (beta session context), `src/routes/auth.ts` (beta login route + OIDC routes for future use), `src/lib/auth.ts` (session CRUD, PostgreSQL), `src/middlewares/authMiddleware.ts` (session loader for Bearer tokens)
...
### 1. Replace beta login with Replit Auth
- Remove the `BETA_USERNAME`/`BETA_PASSWORD` constants and `POST /api/auth/login` route from `src/routes/auth.ts`
- The OIDC routes (GET /api/login, GET /api/callback, etc.) are already in place — just wire them up
- Update `context/AuthContext.tsx` to use `expo-auth-session` (PKCE flow) instead of sessionStorage
- Sessions and the `users` table are already in PostgreSQL — no schema changes needed
